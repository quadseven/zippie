...
def test_an_announced_leg_appears():
    ...
    d.announce(name="operator-iphone", host="10.20.0.151", port=51999, label="iPhone")
    assert [l.name for l in d.live()] == ["operator-iphone"]
    assert d.live()[0].relay_endpoint == "10.20.0.151:51999"
...
def test_a_leg_that_stops_announcing_DISAPPEARS():
    ...
    d.announce(name="co-operator-iphone", host="10.20.0.100", port=51999, lease_s=45)
    assert len(d.live()) == 1
...
def test_renewing_keeps_it_alive():
    ...
    d = DynamicLegs(clock=c)
    for _ in range(10):
        d.announce(name="operator-iphone", host="10.20.0.151", port=51999, lease_s=45)
...
def test_a_changed_address_replaces_the_old_one():
    ...
    d = DynamicLegs(clock=Clock())
    d.announce(name="operator-iphone", host="10.20.0.151", port=51999)
    d.announce(name="operator-iphone", host="10.20.0.188", port=51999)
    live = d.live()
    assert len(live) == 1
    assert live[0].relay_endpoint == "10.20.0.188:51999"
...
def test_a_hostile_name_is_refused():
    ...
    for name in ["", "a", "../../etc", "Name With Caps", "has space",
        ...
        with pytest.raises(ValueError):
            d.announce(name=name, host="10.20.0.5", port=51999)


def test_out_of_range_values_are_refused():
    d = DynamicLegs(clock=Clock())
    with pytest.raises(ValueError):
        d.announce(name="ok-leg", host="10.20.0.5", port=0)
    with pytest.raises(ValueError):
        d.announce(name="ok-leg", host="10.20.0.5", port=99999)
    with pytest.raises(ValueError):
        d.announce(name="ok-leg", host="10.20.0.5", port=51999, tier=0)
...
def test_a_lease_cannot_be_made_effectively_permanent():
    ...
    c = Clock()
    d = DynamicLegs(clock=c)
    d.announce(name="sneaky", host="10.20.0.5", port=51999, lease_s=99999)
    c.t += 301
...
def test_several_phones_coexist():
    d = DynamicLegs(clock=Clock())
    d.announce(name="operator-iphone", host="10.20.0.151", port=51999)
    d.announce(name="co-operator-iphone", host="10.20.0.100", port=51999)
    assert len(d.live()) == 2
...
def test_expiry_happens_on_read_not_on_a_timer():
    ...
    c = Clock()
    d = DynamicLegs(clock=c)
    d.announce(name="operator-iphone", host="10.20.0.151", port=51999, lease_s=10)
    c.t += 11
...
def test_an_announced_leg_joins_the_agents_paths(tmp_path):
    ...
    a.dynamic.announce(name="operator-iphone", host="10.20.0.151", port=51999,
    ...
    assert leg.config.relay_endpoint == "10.20.0.151:51999"
...
def test_an_expired_leg_is_REMOVED_not_left_as_a_down_row(tmp_path):
    ...
    a.dynamic.announce(name="co-operator-iphone", host="10.20.0.100", port=51999, lease_s=45)
    a.reconcile_dynamic_legs()
...
def test_a_static_leg_is_never_touched_by_expiry(tmp_path):
    ...
    a = _agent(tmp_path)
    a.dynamic = DynamicLegs(clock=c)
    a.dynamic.announce(name="operator-iphone", host="10.20.0.151", port=51999, lease_s=10)
    a.reconcile_dynamic_legs()
    c.t += 11
    a.reconcile_dynamic_legs()
    assert [p.name for p in a.paths] == ["ethernet"]
...
def test_a_moved_phone_updates_the_endpoint_rather_than_adding_a_leg(tmp_path):
    ...
    a.dynamic.announce(name="operator-iphone", host="10.20.0.151", port=51999)
    a.reconcile_dynamic_legs()
    a.dynamic.announce(name="operator-iphone", host="10.20.0.188", port=51999)
    a.reconcile_dynamic_legs()

    legs = [p for p in a.paths if p.name == "operator-iphone"]
    assert len(legs) == 1, "the phone appeared twice after moving"
    assert legs[0].config.relay_endpoint == "10.20.0.188:51999"
...
def test_a_rename_SURVIVES_the_next_announcement(tmp_path):
    ...
    """
    a = _agent(tmp_path)
    a.dynamic.announce(name="iphone-3f9a", host="10.20.0.151", port=51999,
                       label="iPhone")
    a.reconcile_dynamic_legs()
